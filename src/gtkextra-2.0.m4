...
dnl AM_PATH_GTK_EXTRA_2_0([MINIMUM-VERSION, [ACTION-IF-FOUND [, ACTION-IF-NOT-FOUND [, MODULES]]]])
dnl Test for GTK_EXTRA+, and define GTK_EXTRA_CFLAGS and GTK_EXTRA_LIBS, if gthread is specified in MODULES, 
dnl pass to pkg-config
dnl
AC_DEFUN([AM_PATH_GTK_EXTRA_2_0],
[dnl 
...
AC_ARG_ENABLE(gtkextratest, [  --disable-gtkextratest       do not try to compile and run a test GTK_EXTRA+ program],
  ...
  min_gtkextra_version=ifelse([$1], ,2.0.0,$1)
  ...
  if test x"$no_gtkextra" = x ; then
    ...
    gtkextra_config_major_version=`$PKG_CONFIG --modversion gtkextra-2.0 | \
           sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\1/'`
    gtkextra_config_minor_version=`$PKG_CONFIG --modversion gtkextra-2.0 | \
           sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\2/'`
    gtkextra_config_micro_version=`$PKG_CONFIG --modversion gtkextra-2.0 | \
           sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\3/'`
...
main ()
{
  ...
  if ((gtkextra_major_version != $gtkextra_config_major_version) ||
    ...
    {
      printf("\n*** 'pkg-config --modversion gtkextra-2.0' returned %d.%d.%d, but GTK_EXTRA+ (%d.%d.%d)\n", 
